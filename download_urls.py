"""
Fetches projects and gathers the valid model's URL
"""
import time
...
if __name__ == "__main__":
    ...
    for i, project in enumerate(projects):
        print(
            f"{_date()} [{i+1}/{len(projects)}] "
            + "Now fetching project {project['project_id']} with {project['model_count']} models."
        )
            ...
            if isinstance(project_summary["models"], list):
                for model in project_summary["models"]:
                    if (
                        ...
                        and "qmean_global" in model.keys()
                    ):
                        coordinates_export_rows.append(
